---
title: "A Study of Eviction Events Based on Spatial Data Analysis"
author: "Zhu Jinzhou"
...
```
To ensure that the data has the same projection coordinate system, we transformed all spatial data into the same coordinate reference system (CRS). We then used the tmap package to visualize eviction events and community areas. The graphic shows the distribution of residential eviction events in 2020 by region.
```{r}
...
ppp(x=points_sub_2020_projected_sp@coords[,1],
    ...
    window =window)
```
I use Ripley's K to explore whether eviction events clustered in certain areas.
```{r}
